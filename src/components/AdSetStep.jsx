...
const AdSetStep = ({ onNext, onBack }) => {
    ...
    useEffect(() => {
        const searchLocations = async () => {
            if (countrySearch.length >= 2) {
                ...
                try {
                    const results = await searchGeoLocations(countrySearch);
    ...
    const fetchPixels = async () => {
        ...
        try {
            ...
            // Don't alert here to avoid spamming if API fails silently
        } finally {
    ...
    const handleSelectExisting = (adset) => {
        ...
        setAdsetData({
            ...
            dailyBudget: (adset.dailyBudget || 0) / 100,
            bidStrategy: adset.bidStrategy || '',
            bidAmount: (adset.bidAmount || 0) / 100,
    ...
    const handleNext = async () => {
        ...
        if (mode === 'new') {
            ...
            if (!hasCountries) {
                alert('Please specify at least one target country');
                return;
            }
            ...
            if (adsetData.optimizationGoal === 'OFFSITE_CONVERSIONS') {
                if (!adsetData.pixelId || !adsetData.conversionEvent) {
                    alert('Pixel ID and Conversion Event are required for conversion optimization');
                    return;
                }
            }
            ...
            if ((adsetData.bidStrategy === 'LOWEST_COST_WITH_BID_CAP' || adsetData.bidStrategy === 'COST_CAP') &&
                ...
                alert('Please enter a valid Bid Amount for the selected strategy');
                return;
            }
            ...
            try {
                ...
                await fetch('http://localhost:3001/api/facebook/adsets', {
                    method: 'POST',
                    ...
                    body: JSON.stringify({
                        ...
                        dailyBudget: campaignData.budgetType === 'ABO' ? adsetData.dailyBudget * 100 : null,
                        bidStrategy: campaignData.budgetType === 'ABO' ? adsetData.bidStrategy : null,
                        bidAmount: adsetData.bidAmount * 100,
                        targeting: {
                            geo_locations: {
                        ...
                        pixelId: adsetData.pixelId,
                        conversionEvent: adsetData.conversionEvent,
                        status: 'PAUSED'
                    })
                });
                                        ...
                                        <div className="flex gap-2">
                                            <div className="relative">
                                                <select
                                                    ...
                                                    className={`appearance-none pl-3 pr-8 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm font-medium ${locationMode === 'include'
                                                        ...
                                                        <label key={platform} className="flex items-center gap-2 cursor-pointer">
                                                            <input
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    if (updated.length === 0) {
                                                                        alert('At least one platform must be selected for Manual Placements.');
                                                                        return;
                                                                    }
