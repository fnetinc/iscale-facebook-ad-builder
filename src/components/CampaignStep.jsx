...
const CampaignStep = ({ onNext, onBack }) => {
    ...
    const handleNext = async () => {
        ...
        if (mode === 'existing') {
            setLoading(true);
            try {
                // Use FB ID as part of local ID to ensure uniqueness and consistency
                const id = `camp_${campaignData.id}`;
                await fetch('http://localhost:3001/api/facebook/campaigns', {
                    method: 'POST',
                    ...
                    body: JSON.stringify({
                        ...
                        dailyBudget: campaignData.dailyBudget * 100,
                        bidStrategy: campaignData.bidStrategy,
                        status: campaignData.status,
                        fbCampaignId: campaignData.fbCampaignId
                    })
                });
                setCampaignData(prev => ({ ...prev, id }));
            } catch (error) {
                console.error('Error importing campaign:', error);
                // Continue as it might be already imported
            }
            setLoading(false);
        }
        ...
        if (mode === 'new') {
            ...
            if (campaignData.budgetType === 'CBO' && (!campaignData.dailyBudget || campaignData.dailyBudget <= 0)) {
                alert('Please enter a valid Daily Budget for CBO campaign');
                return;
            }
        ...
        <div>
            ...
            <div className="mt-8 flex justify-between">
                ...
                <button
                    onClick={handleNext}
                    disabled={loading}
                    className="ml-auto flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
                >
