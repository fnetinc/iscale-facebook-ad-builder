...
import { Plus, X, Copy, Check, Upload, Loader } from 'lucide-react';
...
const WinningAds = () => {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:3001/api/winning-ads/upload', {
                method: 'POST',
                body: formData
            });
    ...
    return (
        <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-8 flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900">Image Templates</h1>
                    <p className="text-gray-600 mt-1">Browse and manage your winning ad templates</p>
                </div>
                <button
                    onClick={() => fileInputRef.current?.click()}
                    disabled={uploading}
                    className="flex items-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-2 rounded-lg hover:from-purple-700 hover:to-pink-700 transition-colors font-medium shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
                                    ...
                                    {selectedTemplate.template_structure && (() => {
                                        ...
                                        return (
                                            <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                                                <h3 className="font-semibold text-purple-900 mb-3">Template Structure</h3>
                                                <div className="space-y-2 text-sm">
                                                    {structure.template_name && (
                                                        <div><span className="text-purple-700 font-medium">Name:</span> {structure.template_name}</div>
                                                    )}
                                                    {structure.layout_type && (
                                                        <div><span className="text-purple-700 font-medium">Layout:</span> {structure.layout_type}</div>
                                                    )}
                                                    {structure.aspect_ratio && (
                                                        <div><span className="text-purple-700 font-medium">Aspect Ratio:</span> {structure.aspect_ratio}</div>
                                                    )}
                                                </div>
                                            </div>
                                        );
